# Relatório Técnico Individual - PC 2 - VM 2

## 🖥️ Informações Gerais da Máquina
* **Responsável pelo Teste:** Andrey
* **Hostname:** g9-pc2-vm2
* **FQDN:** g9-pc2-vm2.grupo9.bsi-26-1.maceio.lab
* **IP Dedicado:** 192.168.26.132
...
1. **Teste 1: Ping por IP** (Alvo: VM 5)
```bash
ping -c 2 192.168.26.133
```

2. **Teste 2: Ping por Hostname** (Alvo: VM 6)
```bash
ping -c 2 g9-pc3-vm2
```

3. **Teste 3: Ping por FQDN** (Alvo: VM 7)
```bash
ping -c 2 g9-pc4-vm1.grupo9.bsi-26-1.maceio.lab
```
...
4. **Teste 4: SSH por IP** (Alvo: VM 8 | Usuário: henrique.carvalho)
```bash
ssh henrique.carvalho@192.168.26.136
```

*<p align="center">Figura 3: Retorno do teste SSH por IP.</p>*![Retorno do teste SSH por IP](./ssh-ip.png)

5. **Teste 5: SSH por Hostname** (Alvo: VM 1 | Usuário: eduardo.calado)
```bash
ssh eduardo.calado@g9-pc1-vm1
```
