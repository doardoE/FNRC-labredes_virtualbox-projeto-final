# Relatório Técnico Individual - PC 4 - VM 1

## 🖥️ Informações Gerais da Máquina
* **Responsável pelo Teste:** Cirilo
* **Hostname:** g9-pc4-vm1
* **FQDN:** g9-pc4-vm1.grupo9.bsi-26-1.maceio.lab
* **IP Dedicado:** 192.168.26.135
...
1. **Teste 1: Ping por IP** (Alvo: VM 8)
```bash
ping -c 2 192.168.26.136
```

2. **Teste 2: Ping por Hostname** (Alvo: VM 1)
```bash
ping -c 2 g9-pc1-vm1
```

3. **Teste 3: Ping por FQDN** (Alvo: VM 2)
```bash
ping -c 2 g9-pc1-vm2.grupo9.bsi-26-1.maceio.lab
```
...
4. **Teste 4: SSH por IP** (Alvo: VM 3 | Usuário: eduardo.calado)
```bash
ssh eduardo.calado@192.168.26.131
```

*<p align="center">Figura 3: Retorno do teste SSH por IP.</p>*![Retorno do teste SSH por IP](./ssh-ip.png)

5. **Teste 5: SSH por Hostname** (Alvo: VM 4 | Usuário: cirilo.silva)
```bash
ssh cirilo.silva@g9-pc2-vm2
```
